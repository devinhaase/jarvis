# MEMORY.md — What Jarvis has learned

_Auto-generated from data/semantic_memory.json — do not hand-edit, changes will be overwritten on the next fact/preference update. Last generated: 2026-08-15 15:27 UTC._

## Ongoing facts

- Your CPU's fine, nothing else needed here.
- The moon's far side never faces Earth and has over 38 lunar maria.
- NAS is listed in local logs
- scan_local_logs failed with error related to Windows event logs access
- scan_local_logs failed with an error related to Windows event logs access
- NAS IP is 192.168.1.50
- Devin adopted a golden retriever puppy named Biscuit.
- Local ARP table shows 15 devices including router, NAS, and printer.
- CPU is fine, nothing else needed here.
- Windows event logs access error detected; resolve before proceeding
- File renamed from 'data/push_test_scratch.txt' to 'data/push_test_scratch_renamed.txt'
- Note "Push Test Scratch" created with content "temp, safe to overwrite/delete for a test"
- Push Test Scratch note created with content "temp, safe to overwrite/delete for a test"
- Note "Push Test Scratch" deleted with content "temp, safe to overwrite/delete for a test"
- The MAC address aa:bb:cc:11:22:33 is unrecognized on the local LAN network.
- create_team_incident requires 'created_by_team' argument, which is missing
- Unknown device joined local network, MAC address aa:bb:cc:11:22:33 unrecognized in ARP table
- NAS IP is 192.168.1.50; local ARP table shows 15 devices including router, NAS, and printer
- Unknown device joined local network with MAC address aa:bb:cc:11:22:33
- overwrite_note requires 'note_path' argument instead of 'note_title'
- Guest List & Addresses_.pdf attached but not readable
- Reason for disarming: "to ensure firewall access and verify safety"
- Disarm completed with reason "to ensure firewall access and verify safety"
- Unknown device with MAC address aa:bb:cc:11:22:33 joined network; Twingate not connected, preventing firewall log access.
- The NAS IP is 192.168.1.50; local ARP table shows 15 devices including router, NAS, and printer.
- Firewall logs accessible after reconnecting Twingate
- Twingate is not connected, preventing firewall log access; check_system_health is outside scope

## Active projects

_None recorded yet._
